...
Block::Block(Function * parent_function, long block_number, long block_starting_line_in_src)
{
	...
	switch(block_number)
	{
		case 0:
		{
			...
			m_block_label = "ENTRY";
			m_statement_list.push_back(new Entry(Location("[" + parent_function->GetDefinitionFilePath() + " : 0]")));
			break;
		}
		case 1:
		{
			...
			m_block_label = "EXIT";
			m_statement_list.push_back(new Exit(Location("[" + parent_function->GetDefinitionFilePath() + " : 0]")));
			break;
		}
		default:
		{
...
bool Block::Parse(std::istream &input_stream)
{
	...
	while(input_stream.good())
	{
		...
		{
			dlog_block << "Found \"SUCC:\" line: \"" << capture_results[1] << "\", ending block "
				<< GetBlockNumber() << ", with "
			...
			if(NumberOfStatements() == 0)
			{
				...
				std::stringstream oss;
				oss << "[" + m_parent_function->GetDefinitionFilePath() + " : " << GetBlockStartingLineNo() << "]";
...
void Block::AddSuccessors(std::string successors_string)
{
	...
	if(successors_string.empty())
	{
		...
		dlog_block << "WARNING: EMPTY SUCCESSOR LIST" << std::endl;
	...
	while(!successors_string.empty())
	{
		...
		else
		{
			...
			// this while loop.
			dlog_block << "WARNING: Unknown successor type in: \"";
			dlog_block << successors_string << "\"" << std::endl;
			break;
		}
	}
}
